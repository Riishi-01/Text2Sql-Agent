...
This module provides a read-only connection to the Olist database:
...
def get_connection_context():
    ...
    Yields:
        psycopg connection with safety settings
    """
    conn = None
    try:
        conn = get_connection()
...
def execute_query(sql: str, row_limit: int = 10000) -> Tuple[List[Dict[str, Any]], List[str]]:
    ...
    with get_connection_context() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(f"SET LOCAL row_security = on")
            cur.execute(sql)
...
def check_read_only_permission() -> bool:
    ...
    try:
        with get_connection_context() as conn:
            with conn.cursor() as cur:
                # Try to check if we're in read-only mode
