...
1. role.md          - static role + hard rules
...
All {{NOW}} placeholders are resolved at runtime from the dataset max date.
"""
import re
...
from agent.schema import get_dataset_max_date
...
def resolve_now(template: str, now_value: str) -> str:
    """Resolve all {{NOW}} placeholders in a template.

    Args:
        template: String containing {{NOW}} placeholders
        now_value: ISO date string to substitute (YYYY-MM-DD)

    Returns:
        Template with all {{NOW}} replaced
    """
    return template.replace("{{NOW}}", now_value)
